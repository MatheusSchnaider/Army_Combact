				...
				switch (type)
				{
					...
					case MESSAGE_TYPE_INSERT_USER_REQUEST:
						if (client != null)
						{
							...
							user.Name = message.GetString("name");
							user.Username = message.GetString("username");
							user.Password = message.GetString("password");
							user.BirthDate = message.GetString("birthDate");
							user.SecurityText = message.GetString("securityText");

							UserBL userBL = UserBL.GetInstance();

							InsertUserResult insertUserResult = userBL.InsertUser(user);

							if (insertUserResult.Type == UserBL.MESSAGE_TYPE_INSERT_USER_SUCCESS)
							{
								client.SendMessage(new
								{
									type = insertUserResult.Type,
									success = insertUserResult.Text
								});
							}
							else
							{
